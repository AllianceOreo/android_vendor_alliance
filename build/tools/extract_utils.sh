...
function setup_vendor() {
    ...
    export ALLIANCE_ROOT="$3"
    if [ ! -d "$ALLIANCE_ROOT" ]; then
        echo "\$ALLIANCE_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$ALLIANCE_ROOT/$OUTDIR" ]; then
        mkdir -p "$ALLIANCE_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$ALLIANCE_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$ALLIANCE_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$ALLIANCE_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ALLIANCE_ROOT"/vendor/alliance/build/tools/smali/baksmali.jar
        export SMALIJAR="$ALLIANCE_ROOT"/vendor/alliance/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$ALLIANCE_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$ALLIANCE_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$ALLIANCE_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$ALLIANCE_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$ALLIANCE_ROOT"/vendor/alliance/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$ALLIANCE_ROOT"/"$OUTDIR"/radio
